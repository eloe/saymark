...
/// Decides, per 160 ms mic chunk, whether to feed it to the STT: skip leading
...
final class SpeechGate {
    ...
    init(
        vad: SileroVAD,
        threshold: Float = 0.5,
        hangoverChunks: Int = MicCapture.speechHangoverChunks
    ) throws {
        self.vad = vad
    ...
    /// Does this 160 ms chunk contain speech? Runs Silero over its 512-sample
    ...
    /// Gate decision for a 160 ms chunk: should the STT consume it?
